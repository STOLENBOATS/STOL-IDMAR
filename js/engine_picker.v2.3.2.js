...
(function(){
  ...
  function attachV1(container, catalog){
    ...
    const boxes={ 
      hp:mk('Potï¿½ncia (hp)','eng_hp'),
      rig:mk('Comando / Rigging','eng_rig'),
      sh:mk('Altura de coluna / Shaft','eng_shaft'),
      rot:mk('Rotaï¿½ï¿½o / Rotation (hï¿½lice / propeller)','eng_rot')
    };
    ...
    else if(Array.isArray(b.power_hp_range)){ hpOptions=[`${b.power_hp_range[0]}ï¿½${b.power_hp_range[1]}`]; }
    ...
    const rotOptions=[
      {value:'STD', label:'STD ï¿½ rotaï¿½ï¿½o normal (clockwise)'},
      {value:'CCW', label:'CCW ï¿½ contra-rotaï¿½ï¿½o (counter-rotating)'}
    ];
  ...
  async function boot(){ 
    ...
    let cat; try{ cat=await loadJSON(url); }catch(e){ console.error('[engine_picker] erro catï¿½logo:', e); return; }
    const target=document.getElementById('brandDynamic'); if(!target){ console.warn('[engine_picker] #brandDynamic nï¿½o encontrado'); return; }
    attachV1(target, cat);

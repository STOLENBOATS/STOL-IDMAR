...
(function(){
  ...
  function mountSelect(labelText, id, options, placeholder="ï¿½ selecione ï¿½"){
  ...
  function fillSelect(sel, options, placeholder="ï¿½ selecione ï¿½", keepValue=false){
  ...
  function attach(container, catalog){
    ...
    if(!selBrand){ console.warn('engine_picker: #brand nï¿½o encontrado'); return; }

    // 2) Famï¿½lia
    const famBox = mountSelect("Famï¿½lia / Family", "eng_family", Object.keys(catalog.brands[selBrand.value]?.families||[]));
    // 3) HP
    const hpBox  = mountSelect("Potï¿½ncia (hp)", "eng_hp", []);
    // 4) Rigging
    const rigBox = mountSelect("Comando / Rigging", "eng_rig", []);
    // 5) Shaft
    const shBox  = mountSelect("Altura de coluna / Shaft", "eng_shaft", []);
    // 6) Rotaï¿½ï¿½o
    const rotBox = mountSelect("Rotaï¿½ï¿½o", "eng_rot", []);
    // 7) Cor
    ...
    function refreshAll(){
      ...
      // valores possï¿½veis gerais da famï¿½lia
      ...
      // liga aos teus campos de pesquisa existentes (para manter histï¿½rico/validaï¿½ï¿½o):
      const mapToExisting = {
        ...
        //.displacement deixamos para futuro (depende da famï¿½lia)
      };

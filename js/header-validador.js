...
(() => {
  ...
  function injectHeader(activeHref){
    ...
    // tenta agarrar botão de tema existente
  ...
  function buildNav(activeHref){
    ...
    [
      ...
      if (href !== '#logout' && (here || ACTIVE) === href.toLowerCase()) {
        a.setAttribute('data-active','1'); a.style.fontWeight='700';
      }
      if (href === '#logout') {
        a.addEventListener('click', (e)=>{
          e.preventDefault();
          try{
            if (window.SupaAuth?.signOut) return void window.SupaAuth.signOut().finally(()=>location.href='login.html');
          }catch(_){}
  ...
  function styleFlex(el, gap){ el.style.display='flex'; el.style.alignItems='center'; el.style.gap=gap||'.5rem'; }

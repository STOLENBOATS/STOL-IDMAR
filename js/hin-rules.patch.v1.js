...
// js/hin-rules.patch.v1.js â€” non-intrusive validator augment (runs after validador-win.js)
(function(){
  const MIN_YEAR = 1998;
  ...
  function parseHIN(raw){
    ...
    const serial = t.slice(5,10);   // positions 6â€“10
  ...
  async function enhance(){
    ...
    function annotate(hinInfo){
      ...
      const items = [];
      if (country) items.push(`<b>PaÃ­s</b>: ${country} (${hinInfo.cc})`);
      if (maker)   items.push(`<b>Fabricante</b>: ${maker} (${hinInfo.mic})`);
      items.push(`<b>Serial (pos. 6â€“10)</b>: ${hinInfo.serial||'â€”'}`);
    ...
    function enforce(hinInfo){
      ...
      let problems = [];
      if (badSerial) problems.push('Serial (pos. 6â€“10) deve ser alfanumÃ©rico (5 chars).');
      if (needsCert && !certOk) problems.push(`Ano < ${MIN_YEAR} sÃ³ Ã© aceite com Certificado de Conformidade.`);

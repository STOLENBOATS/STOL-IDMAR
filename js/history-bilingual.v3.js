...
(function(){
  ...
  function trStatePTEN(text){
    ...
    if (s.includes("pr�-1998") || s.includes("pre-1998")) return 'Pr�-1998 / <span class="en">Pre-1998</span>';
    if (s.includes("v�lido")) return 'V�lido / <span class="en">Valid</span>';
    if (s.includes("inv�lido")) return 'Inv�lido / <span class="en">Invalid</span>';
    return text;
  }
  ...
  function patchTable(tbl){
    ...
    rows.forEach(tr => {
      ...
      const idxState = Array.from(tds).findIndex(td => /v�lido|inv�lido|pr�-1998/i.test(td.textContent));
      if(idxState >= 0){

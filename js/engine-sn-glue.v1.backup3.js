ï»¿(function (w, d) {
  ...
  function ensureSNField(){
    ...
    wrap.innerHTML = [
      '<label for="engine-sn-raw">Nï¿½ do motor / Engine serial</label>',
      '<div class="row">',
  ...
  async function runSNCheck(){
    ...
    // Honda: permitir distinï¿½ï¿½o exterior/interior
    if ((sel.brand||"").toLowerCase() === "honda"){
      // ï¿½Exteriorï¿½ = prefixo+serial tipo BAAL-xxxxxxx; ï¿½Interiorï¿½ = nï¿½ bloco (sï¿½ dï¿½gitos ou padrï¿½o sem prefixo)
      const hasPrefix = /^[A-Z0-9]+[-\s]?\d{4,}$/.test(sn) && /[A-Z]/.test(sn.charAt(0));
      if (kind === "exterior" && !hasPrefix){
        parsed = null;
      }
      if (kind === "interior" && hasPrefix){
        // forï¿½a tratar como bloco interno (sem baliza por prefixo)
    ...
    if (parsed && parsed.brand === "Honda" && parsed.source === "interior"){
      // Sem prefixo: sï¿½ coerï¿½ncia leve (nï¿½o hï¿½ faixa por prefixo)
      const notes = [];
      notes.push("?? Honda (interior): nï¿½mero de bloco interno. Validaï¿½ï¿½o por coerï¿½ncia ï¿½ sem faixa de prefixo.");
      // opcional: cruzar hp/model se soubermos inferir algo do nï¿½ interno (normalmente nï¿½o pï¿½blico)
      res = { ok:true, notes };
    } else {
      ...
      if ((sel.brand||"").toLowerCase()==="honda" && kind!=="interior" && parsed && (!parsed.prefix || !parsed.serial)){
        res.notes = (res.notes||[]).concat(["?? Honda: nï¿½meros exteriores costumam ter prefixo (ex.: BAAL-1234567)."]);
      }
    }
    ...
    if (hintsEl){
      ...
      hintsEl.textContent = bits.join(" ï¿½ ");
    }
  ...
  d.addEventListener("DOMContentLoaded", () => {
    try {
      ...
      console.info("[IDMAR] Engine SN glue ativo (hï¿½brido, page-guarded, Honda dual-SN).");
    } catch(e){}
  });

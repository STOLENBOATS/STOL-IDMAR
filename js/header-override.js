...
(function(w,d){
  ...
  function tweakHeader(){
    ...
    var map = [
      ...
      {href:'historico_win.html', label:'Histï¿½rico WIN'},
      {href:'historico_motor.html', label:'Histï¿½rico Motor'},
      {href:'forense.html', label:'Forense'},
      {href:'login.html', label:'Terminar sessï¿½o', id:'logout'} // fallback if a logout goes to login
    ];
    ...
    $all('nav a, .nav a, .menu a, header a').forEach(function(a){ /*RIBBON_APPLY*/
      try{
        ...
        if(m){ a.textContent = m.label; a.title = m.label; a.setAttribute('aria-label', m.label); a.classList.add('nav-ribbon'); a.dataset.section=(m.id||m.label).toLowerCase().replace(/\s+/g,'_').replace('histï¿½rico_','hist_'); }
      }catch(e){}
    });

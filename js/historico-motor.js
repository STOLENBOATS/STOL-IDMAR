// historico-motor.js — filtros, ordenação e limpeza segura
(function(){'use strict';
  const KEY_NEW = (window.NAV && NAV.STORAGE && NAV.STORAGE.MOTOR_HISTORY) || 'nav_motor_history';
  const KEY_LEG = 'hist_motor';

  function load(key){ try{ return JSON.parse(localStorage.getItem(key)||'[]'); }catch(e){ return []; } }
  function save(key, arr){ localStorage.setItem(key, JSON.stringify(arr||[])); }
  function fmtDate(d){
    try{ if(d==null) return '—';
      if(typeof d==='number' || (typeof d==='string' && /^\d+$/.test(d))){ const n=new Date(Number(d)); return isNaN(n)?'—':n.toLocaleString('pt-PT'); }
      const n=new Date(d); return isNaN(n)?'—':n.toLocaleString('pt-PT'); }catch(e){ return '—'; }
  }
  ...
  if(!controls){
    ...
    controls.innerHTML = '<div class="controls-row">'
      ...
      + '<button id="btnClearMotor" class="btn warn">Limpar histórico</button>'
      + '<button id="exportMotorCsv" class="btn">Exportar CSV</button>'
      + '</div>';
    const firstSection = document.querySelector('.card h2, .card h3, .card .table-wrap, .card table') || card.firstChild;
    if(firstSection && firstSection.parentNode) firstSection.parentNode.insertBefore(controls, firstSection);
  ...
  const btnClr   = document.getElementById('btnClearMotor');
  ...
  function passFilters(rec){
    ...
    if(q){
      const hay = (rec.win+' '+rec.sn+' '+rec.brand+' '+rec.reason).toLowerCase();
  ...
  function render(){
    ...
    tbody.innerHTML = rows.length ? '' : '<tr><td colspan="7" class="small">Sem registos / No records</td></tr>';
    rows.forEach(r=>{
  ...
  if(btnCsv) btnCsv.addEventListener('click', ()=>{
    const rows = [["Data/Date","Marca","S/N","Resultado","Justificação","Foto"]];
    getAll().filter(passFilters).forEach(r=>{
      rows.push([fmtDate(r.date), r.brand||'', r.sn||r.serial||'', r.valid?'Válido':'Inválido', r.reason||'', r.photoName||'']);
    });
    const csv = rows.map(r=>r.map(v=>(''+(v??'')).replace(/\"/g,'\"\"')).map(v=>'\"'+v+'\"').join(',')).join('\\n');
    const blob = new Blob([csv],{type:'text/csv;charset=utf-8;'});
    const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='historico_motor.csv'; document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(()=>URL.revokeObjectURL(url),1000);
  });

  if(btnClr) btnClr.addEventListener('click', ()=>{
    if(!confirm('Apagar todos os registos de Histórico Motor?')) return;
    save(KEY_NEW, []); save(KEY_LEG, []); render();
  });

ï»¿/* IDMAR ï¿½ histï¿½rico WIN (Fase 3)
   - Ordena mais recente ? mais antigo
   - Filtros: estado (Todos/Vï¿½lido/Invï¿½lido) + data (de/atï¿½) + pesquisa (WIN/justificaï¿½ï¿½o)
   - Export CSV (UTF-8 + BOM)
   - Sem mexer no HTML/CSS original; injeta uma toolbar e uma listagem prï¿½pria
*/
(() => {
  ...
  const isTrue = v => v===true || v===1 || v==='1' || v==='true' || v==='Vï¿½LIDO';
  ...
  function buildToolbar(onApply, onClear, onExport){
    ...
    ['Todos','Vï¿½lido','Invï¿½lido'].forEach(v=>{ const o=el('option',null,v); o.value=v.toLowerCase(); status.appendChild(o); });
    ...
    const q = el('input'); q.type='search'; q.placeholder='Pesquisar WIN/justificaï¿½ï¿½oï¿½'; q.setAttribute('data-filter-q','');
  ...
  function applyFilters(rows, f){
    ...
    if (f.status && f.status!=='todos'){
      const wantValid = f.status==='vï¿½lido';
  ...
  function renderItems(listEl, rows){
    ...
    rows.forEach(r=>{
      ...
      const badge = el('strong',null, isTrue(r.valid) ? 'Vï¿½LIDO' : 'INVï¿½LIDO');
  ...
  function toCSV(rows){
    ...
    rows.forEach(r=>{
      const row = [
        ...
        (r.resultado || (isTrue(r.valid)?'Vï¿½LIDO':'INVï¿½LIDO')),
  ...
  function boot(){
    ...
    // primeira renderizaï¿½ï¿½o
    refresh();

    // expï¿½e API mï¿½nima (opcional)

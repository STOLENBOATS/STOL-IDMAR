// engine_validation.v1.js � valida combina��es segundo cat�logo v2 (PT/EN)
(function(w,d){
  ...
  NS.validateSelection = async function(args, opts){
    ...
    let cat=null; try{ cat=await loadJSON((opts&&opts.catalogUrl)||'data/engines_catalog.v2.json'); }catch(e){ return {ok:false, errors:["Cat�logo n�o p�de ser carregado / Catalog could not be loaded"]}; }
    const b=(cat.brands||{})[brand]; if(!b){ return {ok:false, errors:["Marca n�o encontrada / Brand not found"]}; }
    const fam=(b.families||{})[family]; if(!fam){ if(family) errors.push("Fam�lia inv�lida / Invalid family"); return {ok:errors.length===0, errors}; }
    const env=envOf(fam); const varSpec=model? findVariant(fam, model): null;
    if(varSpec && varSpec.hp!=null){ if(hp && String(varSpec.hp)!==hp){ errors.push("Pot�ncia n�o corresponde ao modelo / Power does not match selected model"); } }
    else if(!inSet(hp, env.hp)){ errors.push("Pot�ncia fora da fam�lia / Power not available for this family"); }
    const rigAllowed=varSpec && Array.isArray(varSpec.rigging)&&varSpec.rigging.length? varSpec.rigging: env.rigging;
    if(!inSet(rig, rigAllowed)){ errors.push("Comando inv�lido / Rigging not valid"); }
    const shaftAllowed=varSpec && Array.isArray(varSpec.shaft)&&varSpec.shaft.length? varSpec.shaft: env.shaft;
    if(!inSet(shaft, shaftAllowed)){ errors.push("Altura de coluna inv�lida / Shaft not valid"); }
    const rotAllowed=varSpec && Array.isArray(varSpec.rotation)&&varSpec.rotation.length? varSpec.rotation: env.rotation;
    if(rot && rotAllowed.length && !inSet(rot, rotAllowed)){ errors.push("Rota��o inv�lida / Rotation not valid"); }
    ...
    if(dispAllowed.length && disp && !inSet(disp, dispAllowed)){ errors.push("Cilindrada inv�lida / Displacement not valid"); }

...
(function (w, d) {
  ...
  function readyAfterHeader(){
    var container = $('#app-header') || $('header');
    if(!container) return;
    if(container.children.length){ applyBranding(container); return; }
    var mo = new MutationObserver(function(){
      if(container.children.length){ try{ applyBranding(container); } finally{ mo.disconnect(); } }
    });

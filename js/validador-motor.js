/* MIEC / IDMAR � validador-motor.js (Fase 2: campos din�micos + PT/EN + hist�rico compat)
   Data: 2025-09-18 | JS-only (zero deps). Mant�m HTML/CSS intactos.

   Marcas:
   - YAMAHA (ON): modelo, c�digo (opcional), letra/ano (opcional), n.� s�rie
   - HONDA  (ON): modelo, n.� s�rie
   - SUZUKI (OFF por omiss�o): preparado

   Hist�rico:
   - Escreve em DUAS keys para compat: 'history_motor' e 'historyMotor'
   - Mant�m o registo mais recente no topo
*/
...
(() => {
  ...
  const BRANDS = {
    YAMAHA: {
      ...
      fields: [
        ...
        { id: 'ym',     label: 'Letra/M�s-Ano',  placeholder: 'p.ex. N',       required: false },
    ...
    SUZUKI: {
      enabled: false, // manter off por omiss�o
      label: 'Suzuki',
      fields: [
  ...
  function renderFieldsForBrand(brandKey, brandSelectEl){
    ...
    if (!brand || !brand.enabled){
      const p = mk('p'); p.textContent='Marca n�o dispon�vel [Brand not available]';
  ...
  function validateYamaha(vals){
    ...
    if (!isAlnum(model))  return fail('Modelo com caracteres inv�lidos','Model has invalid chars');
    if (!isAlnum(serial)) return fail('S�rie com caracteres inv�lidos','Serial has invalid chars');
    if (serial.length < 5) return fail('S�rie demasiado curta','Serial too short');
    const heur = /^([A-Z]{1,2}\d{2,3}[A-Z]{0,3})$/.test(model)
              && (!code || /^[A-Z0-9]{2,4}$/.test(code))
              && (!ym   || /^[A-HJ-NPR-Z0-9]{1,3}$/.test(ym));
    if (!heur) return fail('Padr�o Yamaha pouco consistente','Yamaha pattern inconsistent',{model,code,ym,serial});
    return ok('Yamaha v�lido (formato plaus�vel)','Yamaha valid (plausible format)', {brand:'YAMAHA',model,code,ym,serial});
  }
  ...
  function validateHonda(vals){
    ...
    if (!serial) return fail('N.� de s�rie em falta','Missing serial');
    if (!isAlnum(model)) return fail('Modelo com caracteres inv�lidos','Model has invalid chars');
    if (!/^[A-Z0-9-]+$/.test(serial)) return fail('S�rie com caracteres inv�lidos','Serial has invalid chars');
    const heur = /^([A-Z]{1,3}\d{2,3}[A-Z]?)$/.test(model) && serial.length>=6;
    if (!heur) return fail('Padr�o Honda pouco consistente','Honda pattern inconsistent',{model,serial});
    return ok('Honda v�lido (formato plaus�vel)','Honda valid (plausible format)', {brand:'HONDA',model,serial});
  }

  function validateSuzuki(vals){
    const model=upper(vals.model), serial=upper(vals.serial);
    if (!model || !serial) return fail('Modelo/S�rie em falta','Missing model/serial');
    if (!isAlnum(model) || !/^[A-Z0-9-]+$/.test(serial)) return fail('Caracteres inv�lidos','Invalid characters');
    if (serial.length<6) return fail('S�rie demasiado curta','Serial too short');
    return ok('Suzuki (valida��o b�sica ativa)','Suzuki (basic checks only)', {brand:'SUZUKI',model,serial});
  }

  // ---------- Hist�rico (compat total) ----------
  ...
  function recordHistoryMotor(brandKey, values, verdict){
    ...
    const entry = {
      ...
      // campos usados pelos hist�ricos/filtros
      estado: verdict.valid ? 'ok' : 'erro',
      estadoLabel: verdict.valid ? 'V�lido' : 'Inv�lido',
      justificacao: verdict.message || '',
      foto,

      // compat c/ vers�es que liam estas chaves
      valid: !!verdict.valid,
      resultado: verdict.valid ? 'V�LIDO' : 'INV�LIDO',
  ...
  function bootstrap(){
    ...
    function runValidation(ev){
      ...
      if (!brand || !brand.enabled){
        const v = fail('Marca n�o suportada','Brand not supported');
      ...
      // === grava no hist�rico (DUAS keys, topo) ===

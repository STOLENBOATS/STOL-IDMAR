...
(() => {
  ...
  // sess�o simples (para eventualmente ocultar nav se n�o autenticado)
  let authed = true;
  try {
    authed = sessionStorage.getItem('IDMAR_SESSION') === 'ok'
          || sessionStorage.getItem('NAV_SESSION') === 'ok'
          || isLogin; // no login n�o interessa
  } catch(_) {}

  // textos i18n
  const t = (k)=> (window.IDMAR_I18N?.t(k) || k);

  // links de navega��o (s� usados fora do login)
  const HREFS = [
  ...
  // esquerda: logo + t�tulo/subt�tulo
  ...
  // direita: (nav se n�o for login) + toggle idioma + (opcional) toggle tema
  ...
  // Bot�o de idioma PT/EN (sempre vis�vel, inclusive no login)
  ...
  // Bot�o de tema (se existir na p�gina)
  ...
  // aplicar tradu��es ao header
  window.IDMAR_I18N?.apply?.(head);

  // === Normalize de margens/containers homog�neas ===

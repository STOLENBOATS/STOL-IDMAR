...
if(!window.IDMAR_openAttachmentsModal){
  window.IDMAR_openAttachmentsModal = function(record, typeLabel){
    ...
    if(!modal){
      ...
      modal.innerHTML='<div class="idmar-att-backdrop"></div><div class="idmar-att-dialog"><div class="idmar-att-head"><strong id="idmar-att-title">Anexos</strong><button id="idmar-att-close" title="Fechar">ï¿½</button></div><div class="idmar-att-body"></div></div>';
    ...
    modal.querySelector('#idmar-att-title').textContent='Anexos ï¿½ '+(typeLabel||'');
...
function toCSV(rows){ const head=['Data/Hora','WIN / HIN','Estado','Justificaï¿½ï¿½o','Foto']; const lines=[head]; for(const r of rows){ lines.push([fmtTs(r.ts),r.win||'',r.estadoLabel||'',r.justificacao||'',r.foto||'']); } return lines.map(cols=>cols.map(csvCell).join(';')).join('\r\n'); }

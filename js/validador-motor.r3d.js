...
(function(w,d){
  ...
  async function loadCatalog(){ const r=await fetch('data/engines_catalog.v2.json',{cache:'no-store'}); if(!r.ok) throw new Error('catalog'); return r.json(); }
  ...
  function gatherEnv(cat, brand, family){
    ...
    listFam.forEach(f=>{
      ...
      if(Array.isArray(f.years)&&f.years.length===2){ for(let y=f.years[0]; y<=f.years[1]; y++) acc.years.add(String(y)); }
      v.forEach(x=>{
  ...
  async function onSubmit(e){
    ...
    // 2) validação EXTRA dos campos de pesquisa livre vs envelope da marca/família
    try{
    ...
    const rec = {
      ...
      sn: summary, // compatível com coluna "S/N" do histórico
      valid: !!ok,
  ...
  function wire(){
    const form=$id('formMotor'); if(form && !form.dataset.bound){ form.addEventListener('submit', onSubmit); form.dataset.bound='1'; }
  }
